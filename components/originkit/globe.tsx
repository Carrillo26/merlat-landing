...
/** Geographic mask: LATAM approx lat -56..33, lng -118..-34; excludes continental US. */
export function isLatam(lat: number, lng: number): boolean {
    if (lat < -56 || lat > 33) return false;
    if (lng < -118 || lng > -34) return false;
    // Continental US exclusion (rough CONUS west/central band)
    if (lat > 24.5 && lng >= -125 && lng <= -96) return false;
    return true;
}

type LandFeature = GeoJSON.Feature;
...
export default function Globe({
    ...
    useEffect(() => {
        ...
        const loadWorldData = async () => {
            try {
                ...
                if (showOutline && outlineColor && outlineRgba.a > 0) {
                    ...
                    let skippedCount = 0;
                    landFeatures.features.forEach((feature: LandFeature) => {
                        const featureType =
                ...
                if (fill === "solid") {
                    ...
                    const latamCoordinates: number[][] = [];
                    const otherCoordinates: number[][] = [];
                    ...
                    for (let lat = -90; lat <= 90; lat += baseStep) {
                        ...
                        for (let lng = -180; lng < 180; lng += lngStep) {
                            if (allDots || isOnLand(lng, lat)) {
                                if (isLatam(lat, lng)) {
                                    latamCoordinates.push([lng, lat]);
                                } else {
                                    otherCoordinates.push([lng, lat]);
                                }
                            }
                        }
                    }
                    ...
                    if (latamMesh && otherMesh) {
                        // Track secondary mesh on userData for dispose
                        latamMesh.userData.merlatOtherDots = otherMesh;
                    }
                }
        ...
        return () => {
            ...
            const other = dotInstances?.userData.merlatOtherDots as
                | InstancedMesh
                | undefined;
            if (other) {
                other.geometry.dispose();
                (other.material as MeshBasicMaterial).dispose();
                globeGroup.remove(other);
            }
            if (dotInstances) {
                if ((dotInstances as InstancedMesh).geometry)
                    (dotInstances as InstancedMesh).geometry.dispose();
                const mat = (dotInstances as Mesh).material;
